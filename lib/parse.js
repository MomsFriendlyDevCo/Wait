...
export function parseCondition(tokens) {
	...
	let joined = /^[a-z]+$/i.test(tokens[1] || '') && !/^(and|or)$/i.test(tokens[1])
...
export function parseTime(input, now = new Date()) {
	...
	if (clock && (clock.groups.minutes !== undefined || clock.groups.meridian)) {
		...
		let when = new Date(now);
		when.setHours(hours, Number.parseInt(clock.groups.minutes ?? 0), 0, 0);
