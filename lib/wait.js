...
export function waitFor(conditions, options) {
	...
	// Reset + stamp per-condition state so observers (i.e. the display) can track progress
	conditions.flat().forEach(condition => Object.assign(condition, {
		satisfied: false,
		startedAt: Date.now(),
	}));
