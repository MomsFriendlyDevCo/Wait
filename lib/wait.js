...
*/
export async function pollUntil(predicate, interval) {
...
*/
export async function waitForPathChange(path) {
	let signature = await pathSignature(path);
	await pollUntil(async () => await pathSignature(path) != signature, POLL_INTERVALS.path);
}
